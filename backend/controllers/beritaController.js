...
export async function createBeritaManual(req, res) {
  try {
    ...
    const db = await getDb();
    let timestamp = tanggal ? new Date(tanggal).toISOString() : new Date().toISOString();
...
export async function updateBerita(req, res) {
  try {
    ...
    let updatedTanggal = existing.tanggal;
    if (tanggal) {
      updatedTanggal = new Date(tanggal).toISOString().slice(0, 19).replace('T', ' ');
    }
